####Gene Scoring Comparison Between Individuals####

ClusterMarkerComparison <- function(marker.genes.object, seurat.object, object.name.1,object.name.2)
{
  set.seed(1)
  plot.list<-list()
  score.list<-list()
  cell.id<-cbind.data.frame(rownames(seurat.object@meta.data),as.factor(as.character(unlist(seurat.object@ident))))
  colnames(cell.id)<-c("CellName","ClusterID")
  clusters.to.compare<-as.character(unique(marker.genes.object$cluster))
  
  for(plot.iterate in 1:length(clusters.to.compare))
  {
    genes.to.use<-rownames(marker.genes.object)[which(clusters.to.compare[plot.iterate] == marker.genes.object$cluster)]
    
    
    ind1_trimmed_log<-seurat.object@data
    #Cell IDs
    cell.id.trimmed<-cell.id
    ...
    of_interest_genes<-genes.to.use
    
    #Run Scores####
    #of_interest_genes
    of_interest_bin_ID<-colnames(tmp) %in% of_interest_genes #Identify colnames of genes in tmp
    ...
    plot.list[[plot.iterate]]<- q + geom_violin(aes(fill=ClusterID)) + geom_jitter(height=0,color="black") + ggtitle(paste(object.name.2,"Cluster",as.character(clusters.to.compare[plot.iterate]),"Score in",object.name.1,sep=" ")) + theme_bw()
    score.list[[plot.iterate]]<-of_interest_score_plotting_matrix
  }
  return(c(plot.list,score.list))
}
